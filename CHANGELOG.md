...
## v0.51 · 전체 실행 복구
- 배포 저장소의 index.html(v0.48)과 js/app.js(v0.49)가 섞인 상태 및 v0.48에서 발생하는 `pinnedFlashbackSceneId is not defined` 오류를 재현. v0.49 수정본을 기준으로 모든 파일을 v0.51으로 통일.
- 모든 JS/CSS 참조에 동일한 버전 쿼리를 부여하고 이미지 경로도 버전화하여 예전 서비스워커 캐시와 충돌하는 상황을 줄임.
- 자바스크립트 초기화 오류 시 빈 화면만 남는 대신 안내창과 저장 기록을 유지한 캐시 새로고침 기능 제공.
- 서비스워커 설치가 이미지·오디오 하나의 로드 실패 때문에 통째로 중단되지 않도록 핵심 파일만 독립적으로 캐시.
- 프롤로그 자동 자막, 첫 대사, 교사용 장면 점프, 이미지·오디오 파일 연결, 디베이트를 브라우저/정적 검증으로 재점검.

## v0.49
- QA에서 재현된 플래시백 미선언 변수 오류 및 대사창 가림 현상 수정.
- 음량 애니메이션의 범위 이탈, 겹치는 조절 애니메이션과 음악 전환곡 조기 종료 수정.
- scene-37 → 38 귀환 시 흑색 페이드가 밝아지는 연출을 가리지 않도록 중앙 확산 백색 전환 재구성.
- 구형 스크립트가 화면 버전을 v0.32로 덮어쓰는 문제 해결.
- QA 스크립트의 document 미정의, 정적 파일 쿼리 문자열 처리 오류 수정.

## v0.48
- 복구한 v0.45 작업본을 기준으로 새 이미지 팩을 통합.
- 새 캐릭터 3종 반영: 프랑스 장군, 영국 장군, 시녀.
- 새 이벤트 이미지 3종 반영: 앤의 몸에서 눈뜬 루이, 의회를 소집하라, 태양왕의 폭발.
- 「앤의 몸에서 눈뜬 루이」 이벤트 이미지는 흑백 연출로 처리.
- 특수 이벤트 연결 추가: scene-06 시작, scene-09 의회를 소집하라, scene-33 태양왕의 폭발.
- 버전 로그 파일을 하나의 CHANGELOG.md로 통합하고, 낡은 CHANGELOG_v*.md / VALIDATION / DEPLOY 파일은 정리.

## v0.45
- 사용자 제공 OST 8곡을 웹앱에 실제 통합.
- 메인 화면/프랑스: `Le Roi Soleil · 태양왕`.
- 영국 일반 장면/최종 의회: `The Crown Has Rules · 왕관에는 조건이 있다`.
- 꿈 장면: `A Crown in a Dream · 왕관의 꿈`.
- 디베이트 미니게임: `Words Against the Crown · 왕관을 겨눈 논리`.
- 깨달음/후반/엔딩: `Not the King Alone · 왕 혼자였던 것이 아니다`.
- SCENE 33 태양왕의 폭발: `The Sun King Explodes · 태양왕의 폭발`.
- 장소·시간·챕터 전환: `The Passage of Crowns · 왕관이 지나가는 시간`을 짧은 전환 스팅어로 사용.
- 프롱드 회상 구간: `Fronde · A King Remembers` 전용 재생.
- BGM 간 약 1초 크로스페이드 적용. 메인 화면과 프랑스는 같은 곡이므로 게임 시작 시 곡을 처음부터 재시작하지 않고 이어서 재생.
- 전환 스팅어 재생 중 기존 BGM을 자동으로 낮춘 뒤 자연스럽게 복귀하도록 오디오 덕킹 적용.
- 메인 화면에 `음악 ON/OFF` 버튼, 게임 메뉴에 `배경음악 ON/OFF` 버튼 추가. 음악 설정은 저장 상태에 유지.
- 음악 파일은 필요할 때 불러오고 서비스워커가 재생된 파일을 캐시하도록 구성해 초기 로딩 부담을 줄임.


## v0.44
- “영국에서 왕은 군림하되 통치하지 않는다.”를 핵심 대사로 지정해 글자 크기·색·짧은 충격 애니메이션으로 강조.
- 장소/시간 이동 시 검은 전환 화면의 유지 시간을 늘리고 페이드 인/아웃을 느리게 조정.
- SCENE 06에서 앤 여왕의 몸으로 눈을 뜨는 눈꺼풀 개방 연출을 약 3초로 늘려 기상 느낌 강화.
- 디베이트 좌우 인물을 반투명·저채도로 처리하고 중앙 논장/논점 레이어보다 뒤에 배치.
- 기존 디베이트 인물 크기와 움직이는 발언 문장, 논리탄환 조작 방식은 유지.


## v0.43
- 꿈 장면에서는 주인공(앤의 몸에 든 루이)만 컬러로 유지하고 배경·기억 속 인물·꿈 이벤트 이미지는 흑백 처리.
- 대사창/나레이션창의 크기는 유지하면서 이름과 본문을 위쪽으로 이동해 하단 쏠림 완화.
- 프롱드의 난, 찰스 1세의 문, 조건부 왕관, 최종 논쟁의 찰스 1세 회상, 왕을 만든 구조 몽타주, 마지막 거울을 모두 ‘지속형 이벤트 배경’으로 통일.
- 이벤트 이미지가 등장하면 인물·소품·문서·장소 표시는 숨기고, 이벤트 이미지를 메인 배경으로 유지한 채 하단 대사창에서 대화가 계속되도록 변경.
- SCENE 36의 성찰 몽타주에서도 중앙 독백창을 사용하지 않고 이벤트 배경 + 하단 대사창 구조로 진행.
- 이벤트 컷의 일회성 자동 소멸/팝업 소비를 제거하고 관련 대화 구간이 끝날 때까지 유지.
- CHANGELOG는 단일 CHANGELOG.md에 누적.
