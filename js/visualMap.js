...
/* ===== v0.19 PLAYER FLOW / LIMBUS-STYLE LANDSCAPE FRAME =====
   Player-facing behavior patch loaded before app.js. DOM hooks are attached after the page scripts finish. */
(()=>{
  'use strict';
  const ready=fn=>document.readyState==='loading'?document.addEventListener('DOMContentLoaded',fn,{once:true}):setTimeout(fn,0);
  ready(()=>{
    ...
    const specialLayer=document.getElementById('specialLayer');
    ...
    if(!story||!panel||!text||!sceneLabel||!stage)return;

    /* 1) Fullscreen begins at the first legal browser moment: the player's Start/Continue tap. */
    async function enterPlayerFullscreen(){
      ...
      setTimeout(()=>{try{window.scrollTo(0,1)}catch{}},60);
    }
    ['startBtn','continueBtn','restartBtn','endRestartBtn'].forEach(id=>{
      document.getElementById(id)?.addEventListener('click',enterPlayerFullscreen,{capture:true});
    });
    titleScreen?.addEventListener('pointerdown',()=>{document.body.classList.add('flow-start-armed')},{once:true});
    document.addEventListener('fullscreenchange',()=>document.body.classList.toggle('flow-fullscreen',!!document.fullscreenElement));

    /* 2) Limbus-like central composition: narrow the eye travel and cluster the cast automatically. */
    const baseMetrics=window.PORTRAIT_METRICS;
    if(baseMetrics){
      window.portraitFrames=function(cast,width,height){
        const known=cast.filter(e=>baseMetrics[e.id]);
        if(!known.length)return cast.map(()=>null);
        const count=cast.length;
        const cap=count>=3?.45:.61;
        const face=Math.min(height*.168,...known.map(e=>{const m=baseMetrics[e.id];return width*cap*m[2]/m[0]}));
        const eye=Math.max(height*.23,height+10-Math.min(...known.map(e=>{const m=baseMetrics[e.id];return (m[1]-m[3])*face/m[2]})));
        const centers=count===1?{left:.50,center:.50,right:.50}:count===2?{left:.36,center:.50,right:.64}:{left:.27,center:.50,right:.73};
        return cast.map(e=>{
          const m=baseMetrics[e.id];if(!m)return null;
          const scale=face/m[2],center=centers[e.slot]??.5,w=m[0]*scale;
          return {id:e.id,width:w,height:m[1]*scale,left:Math.max(4,Math.min(width-w-4,width*center-m[4]*scale)),top:eye-m[3]*scale,face};
        });
      };
    }

    /* 3) Opening narration: whole sentence -> hold -> fade. Player taps are ignored. */
    const openingBeats=[
      {text:'17세기 후반, 프랑스 베르사유 궁전.',hold:1250},
      {text:'유럽의 강력한 군주 가운데 한 명, 루이 14세. 사람들은 그를 태양왕이라 불렀다.',hold:2300},
      {text:'루이는 세금 징수와 국가 업무를 수행하는 관료제를 강화했다.',hold:2050}
    ];
    let openingBusy=false;
    let openingTimerA=null,openingTimerB=null;
    function currentSceneNumber(){const m=String(sceneLabel.textContent||'').match(/SCENE\s*(\d+)/);return m?Number(m[1]):0}
    function currentOpeningBeat(){
      if(currentSceneNumber()!==1||!panel.classList.contains('narration'))return null;
      const now=String(text.textContent||'').trim();if(!now)return null;
      return openingBeats.find(b=>b.text===now||b.text.startsWith(now))||null;
    }
    function runOpeningBeat(){
      if(openingBusy)return;
      const beat=currentOpeningBeat();if(!beat)return;
      openingBusy=true;story.classList.add('flow-auto-opening');
      // First synthetic click completes the internal typewriter immediately. It does not advance the line.
      panel.click();
      requestAnimationFrame(()=>requestAnimationFrame(()=>story.classList.add('flow-auto-opening-visible')));
      clearTimeout(openingTimerA);clearTimeout(openingTimerB);
      openingTimerA=setTimeout(()=>{
        story.classList.remove('flow-auto-opening-visible');
        openingTimerB=setTimeout(()=>{
          openingBusy=false;story.classList.remove('flow-auto-opening');
          // Second synthetic click advances. Trusted player taps are blocked while auto-opening is active.
          panel.click();
        },330);
      },beat.hold);
    }
    const textObserver=new MutationObserver(()=>runOpeningBeat());
    textObserver.observe(text,{subtree:true,childList:true,characterData:true});
    ['pointerdown','click','touchend'].forEach(type=>story.addEventListener(type,e=>{
      if(story.classList.contains('flow-auto-opening')&&e.isTrusted){e.preventDefault();e.stopImmediatePropagation()}
    },true));

    /* 4) Tap the scenery/character area to advance dialogue too. Controls and cutscenes keep their own taps. */
    story.addEventListener('click',e=>{
      if(!e.isTrusted||story.classList.contains('flow-auto-opening'))return;
      if(e.target.closest('button,dialog,#dialoguePanel,#intertitleOverlay,#specialLayer,.icon-btn,.prop-layer'))return;
      if(!story.classList.contains('active'))return;
      panel.click();
    });

    /* 5) Fronde memory becomes a scene-persistent visual instead of a brief clipped flash. */
    let persistentMemory=null;
    function removePersistentMemory(){
      persistentMemory?.remove();persistentMemory=null;story.classList.remove('flow-persistent-memory');
    }
    function mountFrondeMemory(){
      if(persistentMemory||currentSceneNumber()!==2)return;
      const path=window.resolveSpecialMeta?.('young_louis_fronde')?.path||specialImage?.getAttribute('src');
      if(!path)return;
      const wrap=document.createElement('div');wrap.className='flow-persistent-special';
      const img=document.createElement('img');img.src=path;img.alt='프롱드의 기억';wrap.append(img);stage.append(wrap);
      persistentMemory=wrap;story.classList.add('flow-persistent-memory');
    }
    function syncSpecial(){
      const label=String(specialLabel?.textContent||'');
      if(currentSceneNumber()===2&&label.includes('프롱드')&&!specialLayer?.classList.contains('hidden'))mountFrondeMemory();
    }
    if(specialLayer)new MutationObserver(syncSpecial).observe(specialLayer,{subtree:true,attributes:true,childList:true,attributeFilter:['class','src']});

    /* 6) Anne accession report stays pinned for the whole Anne report scene. */
    let pinnedDoc=null;
    function removePinnedDoc(){pinnedDoc?.remove();pinnedDoc=null;story.classList.remove('flow-pinned-doc-active')}
    function mountAnneDocument(){
      if(pinnedDoc||currentSceneNumber()!==4)return;
      const meta=window.resolveDocumentMeta?.('anne_accession');if(!meta?.path)return;
      const wrap=document.createElement('aside');wrap.className='flow-pinned-document';wrap.setAttribute('aria-hidden','true');
      const img=document.createElement('img');img.src=meta.path;img.alt='';
      const cap=document.createElement('span');cap.textContent='앤 여왕 즉위 보고서';
      wrap.append(img,cap);stage.append(wrap);pinnedDoc=wrap;story.classList.add('flow-pinned-doc-active');
    }
    function syncProp(){
      if(currentSceneNumber()===4&&propCard?.dataset?.prop==='anne_accession'&&!propLayer?.classList.contains('hidden'))mountAnneDocument();
    }
    if(propLayer)new MutationObserver(syncProp).observe(propLayer,{subtree:true,attributes:true,childList:true,attributeFilter:['class','data-prop','src']});

    /* Cleanup persistent overlays only when the actual scene changes. */
    let lastScene=currentSceneNumber();
    new MutationObserver(()=>{
      const now=currentSceneNumber();
      if(now!==lastScene){
        if(lastScene===2)removePersistentMemory();
        if(lastScene===4)removePinnedDoc();
        lastScene=now;
      }
    }).observe(sceneLabel,{subtree:true,childList:true,characterData:true});

    document.title='태양왕이 여왕이 되었다 · PLAYER FLOW v0.19';
    const versionTag=document.querySelector('.version-tag');if(versionTag)versionTag.textContent='GAME / PLAYER FLOW v0.19';
  });
})();
